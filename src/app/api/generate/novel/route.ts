...
type GenerateNovelRequest = {
  avoidances?: string;
  genre?: string;
  existingChapters?: ChapterPlan[];
  protagonist?: string;
  projectTitle?: string;
  setting?: string;
  prompt?: string;
...
type GeneratedChapter = ChapterPlan & {
  content: string;
};
...
function createChapterEnding(content: string) {
  return content.replace(/\s+/g, " ").trim().slice(-520);
}

function getChapterLengthConfig(wordsPerChapter: number) {
...
function buildFallbackPlan(body: Required<GenerateNovelRequest>): PlanResponse {
  return {
    title: "群像传奇",
...
function buildContinuityText(context?: ContinuityContext) {
...
function normalizePlan(plan: PlanResponse | null, body: Required<GenerateNovelRequest>) {
  ...
  return {
    ...
    chapters: Array.from({ length: body.chapterCount }, (_, index) => {
      const fallbackChapter = fallback.chapters?.[index] as ChapterPlan;
...
function normalizeExistingChapters(value: unknown, chapterCount: number) {
  if (!Array.isArray(value)) {
    return [];
  }

  return value
    .map((item): ChapterPlan | null => {
      if (!item || typeof item !== "object") {
        return null;
      }

      const source = item as Partial<ChapterPlan>;
      const chapterNumber = normalizePositiveNumber(source.chapterNumber, 0, 1, chapterCount);
      ...
      return {
        chapterNumber,
        ending: normalizeText(source.ending, ""),
        title: normalizeText(source.title, `第${chapterNumber}章`),
        outline: normalizeText(source.outline, "延续全书主线推进本章剧情。")
      };
    })
    .filter((item): item is ChapterPlan => Boolean(item))
...
async function createChatCompletion(client: OpenAI, messages: OpenAI.Chat.Completions.ChatCompletionMessageParam[], maxTokens: number) {
...
async function generatePlan(client: OpenAI, body: Required<GenerateNovelRequest>) {
...
返回 JSON 格式：
{
  ...
  return normalizePlan(parseJsonObject<PlanResponse>(rawPlan), body);
}

async function generateChapterContent(
  client: OpenAI,
  body: Required<GenerateNovelRequest>,
  planTitle: string,
  allChapters: ChapterPlan[],
  chapter: ChapterPlan,
...
export async function POST(request: Request) {
  try {
    ...
    const rawBody = (await request.json()) as GenerateNovelRequest;
    const body: Required<GenerateNovelRequest> = {
    ...
    const message = error instanceof Error ? error.message : "小说生成失败，请稍后重试。";
